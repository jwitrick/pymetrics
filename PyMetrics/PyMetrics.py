...
def __printSummary( so, context, runMetrics, metrics, pa, html ):
        ...
        for k,t,v in keys:
            if (pa.zeroSw or v):
                if t != NONTOKEN:
                    ...
                    #html and html.write(v)
        print

    if pa.genBasicSw:
        __displayBasicMetrics( keys, pa, so, inFileName, html )

def __displayBasicMetrics( keys, pa, so, inFileName, html ):
...
def __getNewHTMLFile( pa ):
    html = None
    try:
        if pa.htmloutput:
            html = htmlout.HTMLOut(pa.htmloutput_dir, pa.htmloutput_file)
    except StandardError,e:
